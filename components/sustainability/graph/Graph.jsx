...
//   const svgHeight = 375; // height for curve and gradient
//   const yearLabelOffset = 60; // increased space for year labels & border
//   const totalHeight = svgHeight + yearLabelOffset; // total viewBox height

//   // extra padding for line & year text
//   const lineY = svgHeight + 5; // moved line down
//   const yearY = svgHeight + 45; // moved year text down
...
//             {/* Year below curve with extra padding */}
//             <motion.text
//               x={p.x}
//               y={yearY}
//               fill="white"
...
export default function Graph() {
  ...
  const svgWidth = 1340; // total width of the viewBox
  const maxX = points[points.length - 1].x;
  const pathD = points
    .map((p, i) => `${i === 0 ? "M" : "L"} ${p.x},${p.y}`)
    .join(" ");
  ...
  const svgHeight = 375; // height for curve and gradient
  const yearLabelOffset = 60; // space for year labels & border
  const totalHeight = svgHeight + yearLabelOffset; // total viewBox height

  // extra padding for line & year text
  const lineY = svgHeight + 5;
  const yearY = svgHeight + 45;
    ...
    >
      ...
      <defs>
        ...
        <linearGradient id="graphGradient" x1="0" y1="0" x2="0" y2="1">
          <stop offset="5%" stopColor="rgba(172, 56, 217, 0.5)" />
          <stop offset="30%" stopColor="rgba(244,121,34,0.4)" />
          <stop offset="100%" stopColor="rgba(244,121,34,0)" />
        </linearGradient>

        {/* Stroke gradient for the curve line (horizontal) */}
        <linearGradient id="lineGradient" x1="0" y1="0" x2="1" y2="0">
          <stop offset="0%" stopColor="rgba(172,56,217,1)" />
          <stop offset="30%" stopColor="rgba(244,121,34,1)" />
          <stop offset="100%" stopColor="rgba(244,121,34,0.2)" />
        </linearGradient>
      </defs>

      {/* Fill under curve - extend to full width */}
      <motion.path
        d={`${pathD} L ${svgWidth},${svgHeight} L ${points[0].x},${svgHeight} Z`}
        fill="url(#graphGradient)"
        initial={{ opacity: 0 }}
        whileInView={{ opacity: 1 }}
        viewport={{ once: true, amount: 0.1 }}
