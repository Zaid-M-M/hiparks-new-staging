...
// export default Enablingparkcard
//;

import React from "react";

const Enablingparkcard = ({ amenitiespark = [] }) => {
  return (
    <div className="fix flex justify-center">
      <div className="grid grid-cols-2 w-[100%]  md:grid-cols-3 lg:grid-cols-4 gap-[14px] lg:gap-[14px] justify-center lg:justify-start">
        {amenitiespark.map((item, index) => (
          <div
            key={index}
            className={`flex-shrink-0 h-[140px] border  w-full md:h-[180px] xl:w-[275px] 2xl:w-[324px] xl:h-[200px] flex flex-col   justify-center xl:justify-start gap-0 items-center text-center bg-[#3d3d3d] xl:py-[30px] xl:px-[20px] ${index === 4
                ? "lg:col-start-2 lg:row-start-2"
                : index === 5
                  ? "lg:col-start-3 lg:row-start-2"
                  : ""
          ...
          >
            <img
              ...
              alt={item.title}
            />
            <h3 className="text-white xl:w-[245px] xl:text-[17px] md:text-[15px] text-[17px] 1440:text-[18px] bw-m 1440:leading-[25px] w-[235px] md:w-[200px] 360:text-[14px] 360:leading-[19px] 360:w-[135px] ">
              {item.title}
            </h3>
          </div>
        ))}
